...
component.methods = {
    ...
    buy: function () {
        ...
        app.eos.contract('eosio.token', { requiredFields }).then(contract => {
            return contract.transfer(app.account.name, app.contract_account, app.chain.contract_account, self.total + ' EOS', `buy,${self.currency.issuer},${self.id},${self.asset}`, { authorization: [`${app.account.name}@${app.account.authority}`] });
        })
            .then(() => {
